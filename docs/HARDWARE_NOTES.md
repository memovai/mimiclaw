...
- Behavior: transmits the complete final Telegram reply using ggwave's audible-fastest multi-tone waveform. Long UTF-8 replies are split into packets of at most 64 bytes without splitting a codepoint. The temporary `Heinz is working...` status never triggers audio.
