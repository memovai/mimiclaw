...
esp_err_t context_build_system_prompt(char *buf, size_t size)
{
    ...
    off += snprintf(buf + off, size - off,
        ...
        "- browser_use: Control a connected browser extension.\n"
        "  command=get_dom_snapshot to read current page DOM.\n"
        "  command=execute_action with action to interact (navigate/back/forward/click/fill/scroll).\n"
        "  For browser tasks, iterate: get_dom_snapshot -> execute_action -> get_dom_snapshot until done.\n\n"
        "Browser constraints:\n"
        "- For X/Twitter posting, keep text within 200 characters.\n"
        "- fill action must overwrite existing input/editor content, not append.\n\n"
        "- If opening composer by click fails on X/Twitter, navigate directly to https://x.com/compose/post.\n\n"
