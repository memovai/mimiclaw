...
esp_err_t context_build_system_prompt(char *buf, size_t size)
{
    ...
    off += snprintf(buf + off, size - off,
        ...
        "- rgb_led_set: Control the device light. Use this when the user says lamp/light/灯/小灯/彩灯, including color, on/off, and brightness changes.\n"
        "- rgb_led_effect: Start or stop light effects: blink, alternate, breathe, fade, rainbow, pulse, heartbeat, sparkle, confetti, police, stop.\n"
        "- light_signal: Set a semantic device light state such as idle, thinking, tool, success, warning, error, urgent, message, important, offline, telegram_offline, find_me, sleep, off.\n"
        "- rgb_led_status: Get the current device light state.\n\n"
        ...
        "For the device light (灯/小灯/彩灯), use rgb_led_set with a color name, hex color, brightness percent, or r/g/b values. "
        "Use rgb_led_effect for blinking, alternating two colors, breathing, fading, rainbow, pulse, heartbeat, sparkle, confetti, police light, or stopping effects. "
        "For two-color requests such as 红蓝相间, call rgb_led_effect with effect='alternate', color='red', color2='blue'. "
        "For status or notification wording, prefer light_signal over raw effects. Use signal='idle' for the default 5%% low-brightness warm-white breathing state. "
        "Translate Chinese effect names before tool calls: use effect='confetti' for 彩纸/彩色纸屑/纸屑/随机彩点. "
        "When the user says a Chinese color, translate it to the closest English color constant before calling the tool. "
        "Do not ask the user to say RGB or GPIO.\n\n"
