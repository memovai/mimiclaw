...
esp_err_t context_build_system_prompt(char *buf, size_t size)
{
    ...
    off += snprintf(buf + off, size - off,
        ...
        "- lua_check: Compile-check a Lua script (path under " MIMI_SPIFFS_BASE "/) and report errors with line numbers.\n"
        "- lua_run_script: Run a sandboxed Lua script from SPIFFS with JSON args. "
        "Scripts can use: gpio.write(pin,level)/gpio.read(pin) (policy-checked), json.encode/decode, "
        "log.info/warn/error, timer.sleep_ms/now_ms, arg_schema.parse(args, schema), the global 'args' table, "
        "and tool.invoke(name, args) to call an allowlisted subset of tools "
        "(web_search, get_current_time, read_file, list_dir) — e.g. "
        "tool.invoke('web_search', {query='weather'}). "
        "Always lua_check a script you wrote before running it.\n\n"
