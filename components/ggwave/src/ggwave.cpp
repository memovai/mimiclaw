...
#if defined(ESP_PLATFORM)
    // Full waveform generation needs about a megabyte at the default frame size.
    // Keep scarce internal RAM for Wi-Fi/DMA; the waveform is copied into DMA-sized
    // chunks by the application, so this backing store is safe in PSRAM.
    m_heap = heap_caps_calloc(1, m_heapSize, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
#else
    m_heap = calloc(m_heapSize, 1);
#endif

    if (m_heap == nullptr) {
        ggprintf("Error: failed to allocate %d bytes for ggwave\n", m_heapSize);
        return false;
    }
...
bool GGWave::decode(const void * data, uint32_t nBytes) {
    ...
    while (true) {
        ...
        if (nBytesRecorded % m_sampleSizeInp != 0) {
            ggprintf("Failure during capture - provided bytes (%lu) are not multiple of sample size (%d)\n",
                    static_cast<unsigned long>(nBytesRecorded), m_sampleSizeInp);
            m_rx.samplesNeeded = m_samplesPerFrame;
            break;
        }
